...
CREATE POLICY "Admin users can manage integrations" ON integrations
    FOR ALL USING (auth.uid() IN (SELECT id FROM admin_users WHERE status = 'active'));
...
SELECT 
    ...
    u.current_usage,
    u.usage_percentage,
    c.total_cost as last_month_cost,
    CASE 
...
LEFT JOIN integration_costs c ON i.id = c.integration_id 
    AND c.month_year = DATE_TRUNC('month', CURRENT_DATE - INTERVAL '1 month');

-- Create monthly cost summary view
CREATE OR REPLACE VIEW monthly_integration_costs AS
SELECT 
    DATE_TRUNC('month', c.month_year) as month,
    COUNT(*) as active_integrations,
    SUM(c.base_cost) as total_base_cost,
    SUM(c.usage_cost) as total_usage_cost,
    SUM(c.overage_cost) as total_overage_cost,
    SUM(c.total_cost) as total_monthly_cost,
    AVG(c.total_cost) as average_cost_per_integration
FROM integration_costs c
GROUP BY DATE_TRUNC('month', c.month_year)
ORDER BY month DESC;